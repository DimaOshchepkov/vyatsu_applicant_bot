...
class EducationLevelRepositoryImpl(
    BaseRepository[EducationLevelDomain, EducationLevel], EducationLevelRepository
):
    def __init__(self, db: AsyncSession):
        super().__init__(db, EducationLevelDomain, EducationLevel)

    @cached(ttl=600, key_builder=classaware_key_builder)
    async def get_all(self) -> List[EducationLevelDomain]:
